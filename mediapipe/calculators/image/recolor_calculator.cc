...
absl::Status RecolorCalculator::InitGpu(CalculatorContext* cc) {
  ...
  const std::string frag_src = R"(
    ...
    void main() {
      ...
      vec4 texColor = vec4(0.0); // texture2D(frame, sample_coordinate)
//      texColor += texture2D(frame, sample_coordinate - 4.0*blurSize) * 0.016216;
//      texColor += texture2D(frame, sample_coordinate - 3.0*blurSize) * 0.054054;
//      texColor += texture2D(frame, sample_coordinate - 2.0*blurSize) * 0.1216216;
//      texColor += texture2D(frame, sample_coordinate - blurSize) * 0.1945946;
//      texColor += texture2D(frame, sample_coordinate) * 0.227027;
//      texColor += texture2D(frame, sample_coordinate + blurSize) * 0.1945946;
//      texColor += texture2D(frame, sample_coordinate + 2.0*blurSize) * 0.1216216;
//      texColor += texture2D(frame, sample_coordinate + 3.0*blurSize) * 0.054054;
//      texColor += texture2D(frame, sample_coordinate + 4.0*blurSize) * 0.016216;

      texColor += texture2D(frame, sample_coordinate - 4.0 * blurSize) * 0.05;
      texColor += texture2D(frame, sample_coordinate - 3.0 * blurSize) * 0.09;
      texColor += texture2D(frame, sample_coordinate - 2.0 * blurSize) * 0.12;
      texColor += texture2D(frame, sample_coordinate - 1.0 * blurSize) * 0.15;
      texColor += texture2D(frame, sample_coordinate) * 0.16;
      texColor += texture2D(frame, sample_coordinate + 1.0 * blurSize) * 0.15;
      texColor += texture2D(frame, sample_coordinate + 2.0 * blurSize) * 0.12;
      texColor += texture2D(frame, sample_coordinate + 3.0 * blurSize) * 0.09;
      texColor += texture2D(frame, sample_coordinate + 4.0 * blurSize) * 0.05;

      fragColor = mix(color1, texColor,  weight.MASK_COMPONENT);
